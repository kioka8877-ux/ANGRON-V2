...
| Frigate | Nom | Rôle |
|---------|-----|------|
| F01_SANGUIS | **SANGUIS** | Cerveau narratif — génération du script viral |
| F02_LACERAT | **LACERAT** | Traducteur tactique — prompt Manim + sync Whisper |
| F03_CRUOR | **CRUOR** | Moteur de rendu — Manim headless autonome |
| F04_NAILS | **NAILS** | Finisseur — FFmpeg fusion audio/vidéo |
| F05_NUCERIA | **NUCERIA** | Camouflage — wipe métadonnées, zéro fingerprint |
...
├── F01_SANGUIS/                 ← Cerveau narratif
│   ├── CODEBASE/
│   ├── IN/
│   └── OUT/
├── F02_LACERAT/                 ← Traducteur tactique
│   ├── CODEBASE/
│   ├── IN/
│   └── OUT/
├── F03_CRUOR/                   ← Moteur Manim
│   ├── CODEBASE/
│   ├── IN/
│   └── OUT/
├── F04_NAILS/                   ← Finisseur FFmpeg
│   ├── CODEBASE/
│   ├── IN/
│   └── OUT/
├── F05_NUCERIA/                 ← Camouflage métadonnées
│   ├── CODEBASE/
│   ├── IN/
│   └── OUT/
...
```
[Claude actif]   F01_SANGUIS — génération script
[GATE]           opérateur valide le script
[Claude actif]   F02_LACERAT — prompt Manim + Whisper sync
[Claude INACTIF] Whisper tourne seul → timestamps extraits
[GATE]           opérateur valide le prompt
[Claude actif]   F03_CRUOR — génère scene.py + déclenche render.sh
[Claude INACTIF] Docker + Manim → DONE.txt
[GATE]           opérateur valide la vidéo brute
[Claude actif]   déclenche finish.sh
[Claude INACTIF] FFmpeg F04_NAILS → NAILS_DONE.txt
[Claude INACTIF] FFmpeg F05_NUCERIA → NUCERIA_DONE.txt
